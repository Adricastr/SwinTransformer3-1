...
def main():
    # Hyperparameters
    batch_size = 8  # Increased for GPU (RTX 4060 can handle this)
    num_epochs = 50
    learning_rate = 1e-4
    num_frames = 16  # Back to 16 frames for better temporal modeling
    img_size = (224, 224)  # Back to full resolution
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, 
                            shuffle=True, num_workers=4, pin_memory=True)  # Optimized for GPU
    val_loader = DataLoader(val_dataset, batch_size=batch_size, 
                          shuffle=False, num_workers=4, pin_memory=True)  # Optimized for GPU
    
    # Create model - Optimized for RTX 4060 (8GB VRAM)
    model = VideoSwinTransformer(
        img_size=img_size,
        patch_size=(2, 4, 4),
        in_channels=3,
        num_classes=23,  # Based on your id_1 to id_23 folders
        embed_dim=96,  # Restored full capacity
        depths=[2, 2, 6, 2],  # Restored full depth
        num_heads=[3, 6, 12, 24],  # Restored full heads
        window_size=(8, 7, 7),  # Restored full window
        drop_rate=0.1,
        attn_drop_rate=0.1
    ).to(device)
