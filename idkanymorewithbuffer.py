...
def main(resume=False):
    ...
    if torch.cuda.is_available():
        ...
        batch_size = 8  # Increased from 2 (4x more)
        num_frames = 16  # Increased from 8 (2x more)
        img_size = (224, 224)  # Increased from 112 (4x more pixels)
        num_workers = 2  # Reduced from 4 to lower CPU usage
        pin_memory = True
    ...
    num_epochs = 100  # More epochs since we have early stopping
    learning_rate = 5e-5  # Reduced from 1e-4 (was too high!)
    ...
    cache_mode = 'full'  # Recommended default
    ...
    # Create model - Better capacity now that we know GPU can handle more
    model = VideoSwinTransformer(
        img_size=img_size,
        patch_size=(4, 4, 4),  # Back to smaller patches for more detail
        in_channels=3,
        num_classes=23,
        embed_dim=96,  # Back to 96 from 64
        depths=[2, 2, 6, 2],  # Full depth restored
        num_heads=[3, 6, 12, 24],  # Full heads restored
        window_size=(4, 7, 7),  # Larger temporal window
        drop_rate=0.1,
        attn_drop_rate=0.1
    ).to(device)
    ...
    print("Learning rate scheduler: ReduceLROnPlateau (will reduce LR if no improvement)")
