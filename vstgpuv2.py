...
def main():
    ...
    # Create model - Very memory-efficient configuration for 8GB GPU
    model = VideoSwinTransformer(
        img_size=img_size,
        patch_size=(4, 8, 8),  # Even larger patches for fewer tokens
        in_channels=3,
        num_classes=23,
        embed_dim=64,  # Reduced from 96
        depths=[2, 2, 4, 2],  # Reduced middle depth from 6 to 4
        num_heads=[2, 4, 8, 16],  # Reduced from [3, 6, 12, 24]
        window_size=(2, 7, 7),  # Smaller temporal window
        drop_rate=0.1,
        attn_drop_rate=0.1
    ).to(device)
